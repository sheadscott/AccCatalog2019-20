          ...
          <UnorderedList>
            {
              menuData.map((item, i) => {
                if (item.children && item.children.length > 0) {
                  return (
                    <Submenu text={item.title} key={`submenu_${item.object_slug}_${i}`} slug={item.object_slug} open={this.props.currentSlug === item.object_slug}>
                      {this.printList(item.children, null, this.props.link.slice(0, -1).split('/').pop())}
                    </Submenu>
                  );
                }

                return (
                  <li key={`contextMenu_${item.slug}_${i}`} className={this.props.currentSlug === item.object_slug ? 'active' : null}>
                    <ContextMenuItem label={item.title} url={item.url} className={this.props.currentSlug === item.object_slug ? 'active' : null} />
                  </li>
                );
              })
            }
